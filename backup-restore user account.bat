...
echo set source drive like D:\ will pull data from D:\backups\%username% (if network, must be mapped)
SET /p _destination=
...
:: output text logs 
SET _txtoutput="c:\temp\%username%.restore.%today%.txt"
:: Robocopy settings
...
:: /MOVE :: Move files and directories and delete from source after copying
...
::Robocopy %_source2% %_destination%\backups\Public *.* %_options% %_exclude2%
Robocopy %_destination%\backups\%username%\Public %_source2% *.* %_options%
:: User account
::Robocopy %_source% %_destination%\backups\%username% *.* %_options% %_exclude%
Robocopy %_destination%\backups\%username% %_source% *.* %_options%
